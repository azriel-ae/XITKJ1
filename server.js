    ...
    const requested =
        url.pathname === "/"
            ...
            : url.pathname === "/admin"
                ...
                : url.pathname === "/berita"
                    ? "/berita.html"
                    // /berita/:slug -> berita-detail.html (slug dibaca dari path
                    // di sisi client, lihat assets/js/news.js#newsSlugFromPath).
                    // Samakan dengan rewrite di vercel.json untuk deploy.
                    : /^\/berita\/[^/]+\/?$/.test(url.pathname)
                        ? "/berita-detail.html"
