...
// -------------------------------------------------------------
// Logging KHUSUS DEVELOPMENT untuk memantau operasi Blob (put/del/get).
// Tujuan: supaya ke depannya gampang melihat operasi mana yang paling
// sering terjadi (put/copy/list = "Advanced Operations" yang kena limit
// di paket Hobby) tanpa perlu buka dashboard Vercel setiap saat.
//
// Aktif otomatis saat dijalankan lokal (NODE_ENV bukan "production").
// Di deployment Vercel (NODE_ENV="production" otomatis), log ini MATI
// secara default supaya tidak menambah noise/log volume di production.
// Bisa dipaksa aktif di production juga (mis. untuk diagnosis sementara)
// dengan set env var BLOB_DEBUG=1 lewat dashboard Vercel.
//
// Sengaja HANYA mencatat: jenis operasi, key/path, dan pemanggilnya —
// TIDAK PERNAH mencatat isi data (bisa berisi password hash, dsb).
// -------------------------------------------------------------
const BLOB_DEBUG_ENABLED =
    process.env.BLOB_DEBUG === "1" || process.env.NODE_ENV !== "production";

function logBlobOp(op, key, caller) {
    if (!BLOB_DEBUG_ENABLED) return;
    console.log(`[BLOB] ${op} "${key}"${caller ? ` (via ${caller})` : ""} @ ${new Date().toISOString()}`);
}
...
async function uploadImage(folder, filename, buffer, contentType) {
    ...
    if (config) {
        ...
        logBlobOp("put (Advanced Op)", relativePath);
...
async function deleteImage(urlOrPath) {
    ...
    if (/\.blob\.vercel-storage\.com\//.test(urlOrPath)) {
        ...
        if (config) {
            try {
                logBlobOp("del (gratis, tidak masuk Advanced Op)", urlOrPath);
