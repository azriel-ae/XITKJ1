...
// Bentuk data yang aman ditampilkan ke PUBLIK (pengunjung website) dan ke
// akun admin/siswa biasa di dashboard: TIDAK menyertakan identitas
// penulis (authorId/authorName/authorRole) sama sekali. Sesuai
// permintaan: hanya super_admin yang boleh tahu berita ditulis oleh
// siapa — role lain (termasuk admin biasa) tidak diperlihatkan.
...
// Bentuk data untuk dashboard (resource=news di api/admin/gallery.js).
// includeAuthor hanya true untuk super_admin — lihat listNewsForUser.
// authorId TETAP disertakan di sini (dipakai untuk keperluan internal),
// tapi authorName/authorRole (identitas yang terlihat manusia) hanya
// ikut kalau includeAuthor true.
function toManagedShape(item, includeAuthor) {
    const base = toPublicShape(item);
    if (includeAuthor) {
        base.authorName = item.authorName;
        base.authorRole = item.authorRole;
    }
    return base;
}
...
async function listNewsForUser(adminInfo) {
    ...
    // Identitas penulis (authorName/authorRole) hanya ikut ditampilkan
    // kalau yang membuka dashboard adalah super_admin.
    const includeAuthor = adminInfo.role === ROLES.SUPER_ADMIN;
    return sortForDisplay(list).map(item => toManagedShape(item, includeAuthor));
}
...
async function createNews(input, adminInfo) {
    ...
    if (!contentRaw) throw new Error("Isi pengumuman wajib diisi.");
...
async function updateNews(id, input, adminInfo) {
    ...
    if (index === -1) throw new Error("Pengumuman tidak ditemukan (mungkin sudah dihapus).");

    const target = all[index];
    if (!canEditNews(adminInfo, target)) {
        const error = new Error("Anda tidak memiliki izin untuk mengedit pengumuman ini.");
        error.statusCode = 403;
        throw error;
    }

    // Snapshot field-field yang benar-benar bisa berubah lewat form edit,
    // dipakai di akhir untuk cek "apakah benar-benar ada perubahan" —
    // supaya submit tanpa perubahan (mis. admin buka lalu langsung
    // simpan lagi, atau submit ganda) tidak menulis ulang ke Blob
    // (writeJson = 1 Advanced Operation) kalau memang tidak perlu.
    const before = {
        title: target.title,
    ...
    if (input.isPinned !== undefined) {
        if (!canPin(adminInfo)) {
            const error = new Error("Hanya admin/super_admin yang boleh mem-pin pengumuman.");
            error.statusCode = 403;
            throw error;
        }
    ...
    const isUnchanged = Object.keys(before).every(key => before[key] === after[key]);
    if (isUnchanged) {
...
async function deleteNews(id, adminInfo) {
    ...
    if (!target) throw new Error("Pengumuman tidak ditemukan (mungkin sudah dihapus).");

    if (!canEditNews(adminInfo, target)) {
        const error = new Error("Anda tidak memiliki izin untuk menghapus pengumuman ini.");
        error.statusCode = 403;
        throw error;
    }
