...
async function handleData(req, res, adminInfo) {
    ...
    // Optimasi kuota Blob Advanced Operations: kalau data yang dikirim
    // PERSIS SAMA dengan data yang sedang aktif (override yang sudah ada,
    // atau data bawaan kalau belum pernah di-override), tidak perlu
    // menulis ulang ke Blob sama sekali (put() = 1 Advanced Operation).
    // Ini menangani kasus admin buka form edit lalu klik Simpan tanpa
    // mengubah apa pun, atau submit ganda dengan isi yang identik.
    const currentEffective = overrides[absen] || {
        nama: student.nama,
        nis: student.nis,
        jk: student.jk,
        ig: student.ig || ""
    ...
    const isUnchanged =
        currentEffective.nama === nama &&
        currentEffective.nis === nis &&
        currentEffective.jk === jk &&
        (currentEffective.ig || "") === ig;
    if (isUnchanged) {
        return res.status(200).json({ ok: true, absen, nama, nis, jk, ig, unchanged: true });
    }
