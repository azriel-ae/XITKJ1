(function () {
  ...
  const panelMeta = {
    ...
    jadwal: { title: "Jadwal Pelajaran", subtitle: "Kelola jadwal pelajaran per hari (timezone Asia/Jakarta)." },
  ...
  function showDashboard(username, isSuperAdmin, role, assignedAbsen) {
    ...
    // Jadwal Pelajaran: hanya untuk role admin & super_admin (akun siswa
    // tidak diizinkan kelola jadwal, sama seperti ditegakkan di server).
    const navJadwalItem = document.getElementById("navJadwalItem");
    if (currentRole === "siswa") {
      navJadwalItem.hidden = true;
    } else {
      navJadwalItem.hidden = false;
      loadJadwalAdmin();
    }
  ...
  // ---------- JADWAL PELAJARAN ----------
  // Hanya role admin & super_admin yang punya akses ke panel ini (lihat
  // showDashboard di atas & pengecekan role di server, api/admin/gallery.js).
  const DAY_LABELS_JADWAL = {
    senin: "Senin", selasa: "Selasa", rabu: "Rabu", kamis: "Kamis",
    jumat: "Jumat", sabtu: "Sabtu", minggu: "Minggu"
  };
  const DAY_ORDER_JADWAL = ["senin", "selasa", "rabu", "kamis", "jumat", "sabtu", "minggu"];
  let jadwalDataByDay = {};

  async function loadJadwalAdmin() {
    const container = document.getElementById("jadwalAdminList");
    if (!container) return;
    container.innerHTML = `<div class="admin-skeleton-grid">${gallerySkeleton()}</div>`;
    try {
      const data = await api("/api/admin/gallery?resource=jadwal");
      jadwalDataByDay = data.data || {};
      renderJadwalAdminList();
    } catch (error) {
      container.innerHTML = `<div class="empty-state is-error"><i class="fa-solid fa-triangle-exclamation"></i>Data gagal dimuat.<br>
        <button type="button" class="empty-state-retry" data-retry="jadwal"><i class="fa-solid fa-rotate-right"></i> Coba Lagi</button></div>`;
    }
  }
  ...
  function renderJadwalAdminList() {
    ...
    container.innerHTML = DAY_ORDER_JADWAL.map(day => {
      const items = jadwalDataByDay[day] || [];
      const rows = items.length
        ? items.map(item => `
          <div class="admin-news-item" data-jadwal-id="${item.id}">
            <div class="admin-news-item-body">
              <span class="admin-news-item-title">${escapeHtml(item.jam)} — ${escapeHtml(item.mapel)}</span>
              <span class="admin-news-item-meta">${escapeHtml(item.guru)}</span>
            </div>
            <div class="admin-news-item-actions">
              <button type="button" class="admin-btn admin-btn-ghost" data-jadwal-edit="${item.id}" data-jadwal-day="${day}" aria-label="Edit jadwal"><i class="fa-regular fa-pen-to-square"></i></button>
              <button type="button" class="admin-btn-icon-danger" data-jadwal-delete="${item.id}" data-jadwal-day="${day}" aria-label="Hapus jadwal"><i class="fa-solid fa-trash"></i></button>
            </div>
          </div>`).join("")
        : `<p class="empty-state">No Data</p>`;

      return `
        <div class="admin-jadwal-day-group">
          <h3 class="admin-jadwal-day-title">${DAY_LABELS_JADWAL[day]}</h3>
          ${rows}
        </div>`;
    }).join("");
  }

  document.getElementById("jadwalAdminList").addEventListener("click", async event => {
    const retryBtn = event.target.closest("[data-retry]");
    if (retryBtn) { loadJadwalAdmin(); return; }

    const editBtn = event.target.closest("[data-jadwal-edit]");
    if (editBtn) {
      const day = editBtn.dataset.jadwalDay;
      const item = (jadwalDataByDay[day] || []).find(i => i.id === editBtn.dataset.jadwalEdit);
      if (item) openJadwalForEdit(day, item);
      return;
    }

    const deleteBtn = event.target.closest("[data-jadwal-delete]");
    if (deleteBtn) {
      const day = deleteBtn.dataset.jadwalDay;
      const item = (jadwalDataByDay[day] || []).find(i => i.id === deleteBtn.dataset.jadwalDelete);
      if (!item) return;
      const confirmed = await askConfirm({
        title: "Hapus jadwal ini?",
        body: `"${item.mapel}" (${item.jam}) pada hari ${DAY_LABELS_JADWAL[day]} akan dihapus permanen.`,
        okLabel: "Hapus"
      });
      if (!confirmed) return;
      try {
        await api("/api/admin/gallery?resource=jadwal", {
          method: "DELETE",
          body: JSON.stringify({ hari: day, id: item.id })
        });
  ...
  function resetJadwalForm() {
    ...
    document.getElementById("jadwalOriginalHari").value = "";
    document.getElementById("jadwalFormTitle").textContent = "Tambah Jadwal";
    document.getElementById("jadwalSubmitLabel").textContent = "Tambah Jadwal";
    document.getElementById("jadwalCancelEdit").hidden = true;
    setStatus(document.getElementById("jadwalStatus"), "", null);
  }

  document.getElementById("jadwalCancelEdit").addEventListener("click", resetJadwalForm);

  function openJadwalForEdit(day, item) {
    document.getElementById("jadwalId").value = item.id;
    document.getElementById("jadwalOriginalHari").value = day;
    document.getElementById("jadwalHari").value = day;
    document.getElementById("jadwalJam").value = item.jam;
    document.getElementById("jadwalMapel").value = item.mapel;
    document.getElementById("jadwalGuru").value = item.guru;
    document.getElementById("jadwalFormTitle").textContent = `Edit Jadwal — ${item.mapel}`;
    document.getElementById("jadwalSubmitLabel").textContent = "Simpan Perubahan";
    document.getElementById("jadwalCancelEdit").hidden = false;
    setStatus(document.getElementById("jadwalStatus"), "", null);
    goToPanel("jadwal");
  ...
  jadwalForm.addEventListener("submit", async event => {
    ...
    const jam = document.getElementById("jadwalJam").value.trim();
    const mapel = document.getElementById("jadwalMapel").value.trim();
    const guru = document.getElementById("jadwalGuru").value.trim();
    const hari = document.getElementById("jadwalHari").value;

    if (!jam) return setStatus(statusEl, "Jam wajib diisi.", "error");
    if (!mapel) return setStatus(statusEl, "Mata pelajaran wajib diisi.", "error");
    if (!guru) return setStatus(statusEl, "Nama guru wajib diisi.", "error");

    const id = document.getElementById("jadwalId").value;
    const originalHari = document.getElementById("jadwalOriginalHari").value;
    const submitBtn = document.getElementById("jadwalSubmit");
    toggleSpinner(submitBtn, true);
    setStatus(statusEl, "", null);
    ...
    try {
      if (id) {
        await api("/api/admin/gallery?resource=jadwal", {
          method: "PUT",
          body: JSON.stringify({ hari: originalHari, id, jam, mapel, guru, newHari: hari })
        });
        showToast("Jadwal berhasil diperbarui.", "success");
      } else {
        await api("/api/admin/gallery?resource=jadwal", {
          method: "POST",
          body: JSON.stringify({ hari, jam, mapel, guru })
        });
